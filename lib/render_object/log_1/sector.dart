...
class SectorConstraints extends Constraints {
  ...
  @override
  bool get isTight =>
      minDeltaTheta >= maxDeltaTheta && minDeltaRadius >= maxDeltaRadius;
...
class RenderSectorRing extends RenderSectorWithChildren {
  ...
  void performLayout() {
    ...
    double innerTheta = paddingTheta;
    double remainingDeltaTheta =
    ...
    while (child != null) {
      ...
      final SectorChildListParentData childParentDate =
          child.parentData! as SectorChildListParentData;
      child = childParentDate.nextSibling;
  ...
  // each sector then knows how to paint itself as its location
  @override
  void paint(PaintingContext context, Offset offset) {
...
class RenderSectorSlice extends RenderSectorWithChildren {
  ...
  RenderSectorSlice({
    ...
    double padding = 0.0,
        ...
        assert(deltaRadius >= 0.0),
        _desiredDeltaTheta = deltaRadius,
        super(decoration);
  ...
  SectorDimensions getIntrinsicDimensions(
    ...
    double remainingDeltaRadius = constraints.maxDeltaRadius - (padding * 2);
    RenderSector? child = firstChild;
    while (child != null) {
      final SectorConstraints innerConstraints = SectorConstraints(
...
class RenderBoxToRenderSectorAdapter extends RenderBox
  ...
  void performLayout() {
    ...
    final double maxChildDeltaRadius = math.min(
      constraints.maxWidth,
      constraints.maxHeight / 2.0 - innerRadius,
    );
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (child != null) {
      ...
      // move the offset to the center of the circle
...
RenderBox buildSectorExample() {
  ...
  rootCircle.add(RenderSolidColor(
  ...
  stack.add(RenderSolidColor(
    const Color(0xFFFFFF00),
    desiredDeltaRadius: 20,
  ));
  stack.add(RenderSolidColor(
